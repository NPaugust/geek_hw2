//Дедлайн до следующего урока
   ...
   //который умеет создавать объекты класса 2й, 3й и 4й и после создания и задания свойств объекту метод
   // возвращает ссылку на объект (пульт). Можно использовать switch для того чтоб определить какого типа нужно создать
...
public class Main {
    ...
    public static void main(String[] args) {
        // method createObject
        createObject("One").print();
        createObject("Two").print();
        createObject("Three").print();
    }
    public static Parent createObject(String className) {
        switch (className) {
            case "One":
                One one = new One(1, "One","First");
                return one;
            case "Two":
                Two two = new Two(2, "Two", "Second");
                return two;
            case "Three":
                Three three = new Three(3, "Three", 3333);
                return three;
        }
        return null;
